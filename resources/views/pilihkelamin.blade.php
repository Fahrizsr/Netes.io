...
<body>
  <div class="container">
    ...
    <form method="POST" action="{{ route('jeniskelamin.update') }}" id="genderForm">
      @csrf
      ...
      <button type="button" class="btn-save" onclick="saveAndSubmit()">Simpan</button>
    </form>
  ...
  <script>
      ...
      if (savedGender) {
        document.querySelectorAll('.option').forEach(option => {
          if (option.querySelector('h3').innerText === savedGender) {
            ...
            document.getElementById('selectedGender').value = savedGender;
          }
        });
      }
    }
    ...
    function saveAndSubmit() {
      ...
      if (selected) {
        ...
        document.getElementById('submittedAt').value = currentDateTime;
